...
void Player::Aim(RenderWindow &rw){
    //diferencia entre posicion mouse respecto a ventana y sprite respecto a ventana
    //coord rectangulares a coord polares x180 / pi
...
pair<bool, Vector2f> Player::AnswerShoot(RenderWindow &rw){
    ...
    Vector2f dirNorm=Vector2f(dir.x/sqrt(pow(dir.x,2)+pow(dir.y,2)),dir.y/sqrt(pow(dir.x,2)+pow(dir.y,2)));     //direccion normalizada
